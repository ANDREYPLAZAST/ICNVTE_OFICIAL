...
export default function InspectionTable({ 
  ...
  const updateTestStatus = async (
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/inspection/${id_team}/update-test`, {
        method: 'PUT',
        headers: {
  ...
  const saveAllEvaluation = async () => {
    setSavingAll(true);
    try {
      // Preparar datos en el formato correcto para el backend
      const sectionsData: { [key: string]: { [key: string]: any } } = {};
      
      sections.forEach(section => {
        sectionsData[section.section_id] = {};
        section.tests.forEach(test => {
          sectionsData[section.section_id][test.test_id] = {
            status_team: test.status_team,
      ...
      const response = await fetch(`${BACKEND_URL}/api/inspection/${id_team}/save`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className="dashboard-card">      <div className="flex items-center justify-between mb-6">
        <div>
        ...
        <div className="flex items-center space-x-4">
          {/* Botón Save All para jueces */}
          {!readOnly && (
            <button
              onClick={saveAllEvaluation}
              disabled={savingAll}
      ...
      {/* Botón para guardar toda la evaluación */}
      {!readOnly && (
        <div className="mt-8">
          <button
            onClick={saveAllEvaluation}
